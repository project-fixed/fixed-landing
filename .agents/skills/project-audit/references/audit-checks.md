...
| TESTS            | Test framework detected       | `Test-Path "vitest.config._" -or Test-Path "jest.config._" -or Test-Path "playwright.config.\*" -or (Get-Content "package.json" | Select-String "vitest                                         | jest                    | playwright | mocha | ava")` | Projects with src/ |
